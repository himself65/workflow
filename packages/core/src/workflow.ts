...
async function createWorkflowSession({
  ...
  workflowContext.promiseQueue = workflowContext.promiseQueue.then(async () => {
    // Preparation may already be running in parallel with VM bootstrap. Keep
    // the blocking input boundary as one span; replay.prepare_payloads shows
    // the background decrypt/decompress pipeline separately.
