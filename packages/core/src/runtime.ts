            ...
            async () => {
              ...
              return trace(
                ...
                async (span) => {
                  ...
                  let streamedPreparationStartedAt: number | undefined;
                  const observeReplayEvent = createReplayEventObserver({
                    runId,
                    cache: replayPayloadCache,
                    resolveKey: resolveReplayKey,
                    preparationStarted: () => {
                      streamedPreparationStartedAt ??= Date.now();
                    },
                  });
                  ...
                  while (true) {
                    ...
                    try {
                      ...
                      const preparationStartedAt = streamedPreparationStartedAt;
                      streamedPreparationStartedAt = undefined;
                      const payloadPrewarm = retainedSession
                        ? replayPayloadCache.prewarm(workflowRun, replayEvents)
                        : preparationStartedAt === undefined
                          ? trace(
                              'workflow.replay.prepare_payloads',
                              async () =>
                                replayPayloadCache.prewarm(
                                  workflowRun,
                                  replayEvents
                                )
                            )
                          : replayPayloadCache
                              .prewarm(workflowRun, replayEvents)
                              .then(() =>
                                recordElapsedSpan(
                                  'workflow.replay.prepare_payloads',
