...
`;
    ...
    // Full rebuild starts: pre-build snapshots capture what the build reads.
    ...
    // The edit lands mid-rebuild; the post-rebuild refresh reads it from disk.
    sources.set(stepFile, postEditSource);
    const sourceSnapshots = new Map<string, SourceSnapshot>([
      [stepFile, await readSnapshot(stepFile)],
      [pageFile, await readSnapshot(pageFile)],
    ]);
...
`;
    ...
    // Full rebuild starts (triggered by the edit that wrote `source`); the
    // pre-build capture reads that same content.
    const preBuildSnapshots = await captureSourceSnapshots({
      discoveredEntries,
      inputFiles: [pageFile],
      readSnapshot,
    ...
    // A duplicate watcher event for the triggering edit arrives mid-rebuild;
    // the post-rebuild refresh reads the same content the build consumed.
    const sourceSnapshots = new Map<string, SourceSnapshot>([
      [stepFile, await readSnapshot(stepFile)],
      [pageFile, await readSnapshot(pageFile)],
    ]);
